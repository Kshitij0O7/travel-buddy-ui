import { TRIP_STYLES } from "../../constants/travel";
...
type Props = {
  ...
  onTripStyle: (style: string) => void;
};

export function TripPlanForm({ form, today, formError, error, onChange, onSubmit, onTripStyle }: Props) {
  return (
    <>
      ...
      <div className="relative z-[1] w-full max-w-[680px] rounded-sm border border-tb-border bg-gray-900/[0.88] p-8 backdrop-blur-xl before:absolute before:inset-x-0 before:top-0 before:h-0.5 before:bg-gradient-to-r before:from-transparent before:via-tb-amber before:to-transparent before:content-[''] max-[600px]:p-8 md:p-12">
        <form onSubmit={onSubmit}>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[0.7rem] font-normal uppercase tracking-[0.2em] text-tb-amber">Children</label>
          ...
          <div className="my-7 h-px bg-tb-border" />

          <div>
            <span className="mb-2 block text-[0.7rem] uppercase tracking-[0.2em] text-tb-amber">Trip style</span>
            <div className="flex flex-wrap gap-2">
              {TRIP_STYLES.map((s) => (
                <button
                  key={s}
                  type="button"
                  className={`cursor-pointer rounded-full border px-4 py-1.5 font-body text-[0.8rem] tracking-wide transition-colors ${
                    form.tripStyle === s
                      ? "border-tb-amber bg-tb-amber-dim text-tb-amber-light"
                      : "border-amber-500/25 bg-transparent text-tb-muted hover:border-tb-amber hover:text-tb-amber-light"
                  }`}
                  onClick={() => onTripStyle(s)}
                >
                  {s}
                </button>
              ))}
